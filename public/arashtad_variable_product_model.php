...
/**
 * Add Variable Product 3D Model in woocommerce single product page
 *
 * @since 1.0
 */

function arashtad_variable_product_model_scripts () {
    ...
    foreach ( $variations as $variation ) {
        ...
        if (empty($model_path) || is_wp_error(wp_remote_head($model_path)) || wp_remote_retrieve_response_code(wp_remote_head($model_path)) !== 200) {
            $metaData[] = [
                'variationId' => $id,
                'type' => 'error',
                'message' => __('The requested 3D model could not be located.', 'arashtad-3d-product-viewer'),
            ];
            continue;
        }
    ...
    wp_enqueue_script('arashtad-variable-product-script', ARASHTAD_3D_PRODUCT_URL . 'public/assets/js/arashtad-variable-product-script.js', array('jquery'), null, true);
    wp_localize_script('arashtad-variable-product-script', 'metaData', $metaData);
...
add_action('wp_enqueue_scripts', 'arashtad_variable_product_model_scripts');
